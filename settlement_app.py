...
# DB에 정산 결과 저장
def save_settlement_to_db(name, date, total_amount, member_count, settlement_data):
    conn = sqlite3.connect('settlement.db')
    c = conn.cursor()
    
    c.execute('''INSERT INTO settlements 
                 (name, date, total_amount, member_count, settlement_data, created_at)
                 VALUES (?, ?, ?, ?, ?, ?)''',
              (name, date, total_amount, member_count, json.dumps(settlement_data), datetime.now().isoformat()))
...
def calculate_settlement():
    ...
    # 평균 금액 계산
    avg_amount = total_amount / len(all_members) if all_members else 0
    
    # 각 멤버별 정산 금액 계산 (평균 - 개인 지출)
    settlement_result = {}
    for member in all_members:
        settlement_amount = avg_amount - member_totals[member]
        settlement_result[member] = {
            'total_spent': member_totals[member],
            'settlement_amount': round(settlement_amount, 0),
            'transactions': []
        }
...
def main():
    ...
    with tab1:
        ...
        # 금액과 날짜를 한 줄에 배치 (모바일에서는 세로로)
        col1, col2 = st.columns([2, 1])
        with col1:
            amount = st.number_input("총 금액", value=default_amount, min_value=0, placeholder="금액을 입력하세요", key="amount_input")
        with col2:
            # 날짜 수정 가능하게 변경
        ...
        if st.session_state.members:
            ...
            if amount > 0 and st.session_state.members:
                ...
                with col1:
                    st.markdown(f'<div class="metric-card"><h4>총 금액</h4><h2>{amount:,}원</h2></div>', unsafe_allow_html=True)
                with col2:
                    st.markdown(f'<div class="metric-card"><h4>참여자 수</h4><h2>{len(st.session_state.members)}명</h2></div>', unsafe_allow_html=True)
                with col3:
                    st.markdown(f'<div class="metric-card"><h4>1인당 금액</h4><h2>{amount_per_person:,.0f}원</h2></div>', unsafe_allow_html=True)
                
                # 참여자별 금액 표시 및 수정 - 모바일 친화적 레이아웃
                st.write("**참여자별 금액:**")
                total_modified = 0
                modified_amounts = []
                ...
                for i, member in enumerate(st.session_state.members):
                    with st.container():
                        # 모바일에서는 세로로 배치
                        st.markdown(f'<div class="member-card">👤 {member}</div>', unsafe_allow_html=True)
                        ...
                        with col2:
                            total_modified += modified_amount
                            st.markdown(f'<div style="text-align: center; padding: 0.5rem; background: #f8f9fa; border-radius: 6px; font-weight: bold;">{total_modified:,}원</div>', unsafe_allow_html=True)
                
                # 최종 금액 비교 - 개선된 표시
                st.markdown("---")
                col1, col2 = st.columns(2)
                with col1:
                    st.write(f"**최종 입력 금액**: {total_modified:,}원")
                with col2:
                    st.write(f"**총 금액**: {amount:,}원")
            ...
            for i, member in enumerate(st.session_state.members):
                ...
                with col1:
                    st.markdown(f'<div class="member-card">👤 {member}</div>', unsafe_allow_html=True)
                with col2:
                    if st.button(f"🗑️", key=f"delete_{i}", use_container_width=True):
        ...
        if st.button(save_button_text, type="primary", disabled=not (description and amount > 0 and st.session_state.members), use_container_width=True):
            if description and amount > 0 and st.session_state.members:
                ...
                if abs(total_modified - amount) > 1:  # 1원 오차 허용
                    st.error(f"참여자별 금액의 합({total_modified:,}원)이 총 금액({amount:,}원)과 일치하지 않습니다!")
                else:
                    if st.session_state.editing_transaction:
                        # 수정 모드
    ...
    with tab2:
        ...
        if not st.session_state.transactions:
            ...
            if settlement:
                ...
                total_spent = sum(data['total_spent'] for data in settlement.values())
                avg_amount = total_spent / len(settlement) if settlement else 0
                
                col1, col2, col3 = st.columns(3)
                with col1:
                    st.markdown(f'<div class="metric-card"><h4>총 거래 금액</h4><h2>{total_spent:,.0f}원</h2></div>', unsafe_allow_html=True)
                with col2:
                    st.markdown(f'<div class="metric-card"><h4>평균 금액</h4><h2>{avg_amount:,.0f}원</h2></div>', unsafe_allow_html=True)
                with col3:
                    st.markdown(f'<div class="metric-card"><h4>참여자 수</h4><h2>{len(settlement)}명</h2></div>', unsafe_allow_html=True)
                ...
                for member, data in settlement.items():
                    ...
                    with st.expander(f"{status_icon} **{member}** - 총 지출: {data['total_spent']:,.0f}원"):
                        # 모바일에서는 세로로 배치
                        st.markdown(f"""
                        <div style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); color: white; padding: 1rem; border-radius: 12px; margin-bottom: 1rem;">
                            ...
                            <div style="display: flex; justify-content: space-between; margin-bottom: 0.5rem;">
                                <span>총 지출:</span>
                                <span><strong>{data['total_spent']:,.0f}원</strong></span>
                            </div>
                            <div style="display: flex; justify-content: space-between; margin-bottom: 0.5rem;">
                                <span>평균 금액:</span>
                                <span><strong>{avg_amount:,.0f}원</strong></span>
                            </div>
                            <div style="display: flex; justify-content: space-between; border-top: 1px solid rgba(255,255,255,0.3); padding-top: 0.5rem;">
                                <span>정산 금액:</span>
                                <span style="color: {color}; font-weight: bold; font-size: 1.1em;">{sign}{data['settlement_amount']:,.0f}원</span>
                            </div>
                        </div>
                        """, unsafe_allow_html=True)
                        ...
                        if data['transactions']:
                            ...
                            for trans in data['transactions']:
                                ...
                                <div style="background: white; padding: 0.8rem; border-radius: 8px; border-left: 4px solid #667eea; margin: 0.3rem 0;">
                                    ...
                                    <div style="color: #667eea; font-weight: bold; margin-top: 0.3rem;">{trans['amount']:,.0f}원</div>
                                </div>
                                """, unsafe_allow_html=True)
    ...
    with tab3:
        ...
        if not settlements:
            ...
            for settlement in settlements:
                with st.expander(f"📅 {settlement['date']} - {settlement['name']} ({settlement['total_amount']:,.0f}원)"):
                    # 삭제 확인 버튼 추가
                    ...
                    col1, col2, col3 = st.columns(3)
                    with col1:
                        st.markdown(f'<div class="metric-card"><h4>총 금액</h4><h2>{settlement["total_amount"]:,.0f}원</h2></div>', unsafe_allow_html=True)
                    with col2:
                        st.markdown(f'<div class="metric-card"><h4>참여자 수</h4><h2>{settlement["member_count"]}명</h2></div>', unsafe_allow_html=True)
                    with col3:
                        avg_amount = settlement['total_amount'] / settlement['member_count'] if settlement['member_count'] > 0 else 0
                        st.markdown(f'<div class="metric-card"><h4>평균 금액</h4><h2>{avg_amount:,.0f}원</h2></div>', unsafe_allow_html=True)
                    ...
                    for member, data in settlement['settlement_data'].items():
                        ...
                        <div style="background: white; padding: 1rem; border-radius: 8px; border-left: 4px solid {color}; margin: 0.5rem 0;">
                            <div style="display: flex; justify-content: space-between; align-items: center;">
                                <div>
                                    <h4 style="margin: 0;">{status_icon} {member}</h4>
                                    <p style="margin: 0.3rem 0; color: #666;">총 지출: {data['total_spent']:,.0f}원</p>
                                </div>
                                <div style="text-align: right;">
                                    <div style="font-size: 1.2em; font-weight: bold; color: {color};">
                                        {sign}{data['settlement_amount']:,.0f}원
                                    </div>
                                    <div style="font-size: 0.9em; color: #666;">
                                        {status_icon} {("받을 금액" if data['settlement_amount'] >= 0 else "낼 금액")}
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        if data['transactions']:
                            ...
                            for trans in data['transactions']:
                                ...
                                <div style="background: #f8f9fa; padding: 0.6rem; border-radius: 6px; margin: 0.2rem 0;">
                                    <div style="font-weight: bold; color: #333;">{trans['description']}</div>
                                    <div style="color: #666; font-size: 0.9em;">{trans['date']}</div>
                                    <div style="color: #667eea; font-weight: bold; margin-top: 0.2rem;">{trans['amount']:,.0f}원</div>
                                </div>
                                """, unsafe_allow_html=True)
